...
async function main(): Promise<void> {
  ...
  // The serverless world API bundles the same record as a TS module — a
  // serverless function cannot read public/ from disk, and a JSON import
  // fights module resolution; a generated module does neither.
  writeFileSync(
    join(STUDIO_PUBLIC, '..', 'api', 'v1', 'world-data.ts'),
    `// GENERATED by still/scripts/slim-world.ts — the deployed world record.\n` +
    `// Do not edit by hand; re-run the script after any world change.\n` +
    `export default ${JSON.stringify(doc)} as unknown as Record<string, unknown>\n`,
  )
  console.log(`record ${Math.round(before / 1024)}KB → ${Math.round(after / 1024)}KB · ${seen.size} seed files · still-world.json + api/v1/world-data.ts written`)
